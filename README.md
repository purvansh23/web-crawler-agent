...
* **The Smart Web-Crawler (HTTPx + Playwright):** The crawler routes its strategy dynamically to maximize speed:
    1. **Sitemap Parsing:** It first aggressively requests `/sitemap.xml` (or `sitemap_index.xml`). If the site owner provides this Google index map, the crawler instantly rips the exact priority pages without needing to click blindly.
    2. **BFS Fallback (Breadth-First Search):** If no sitemap exists (or it 404s), the crawler falls back to navigating the homepage natively, scanning DOM anchor tags (`href`) to build its own localized index of internal links.
    3. **Playwright Bypasser:** Attempts lightning-fast HTTP reads first. If it detects an Anti-Bot Wall (Cloudflare 403) or an empty Javascript framework (React SPA payload), it completely abandons `HTTPx` and intelligently spins up an invisible **Headless Google Chrome (Playwright)** subprocess to visually bypass the block and render the exact DOM text perfectly.
